...
void llama_memory_recurrent::copy_cell(int32_t i_src, int32_t i_dst) {
    ...
    for (uint32_t il = 0; il < hparams.n_layer; ++il) {
        if (r_l[il]) {
            ggml_context * ctx = ggml_init(params);
            // Tensor is 2D: [n_embd_r, mem_size]. Each cell = one row = ne[0] elements.
            int64_t cell_elements = r_l[il]->ne[0];
            size_t  cell_bytes    = r_l[il]->nb[1]; // row stride in bytes
        ...
        if (s_l[il]) {
            ggml_context * ctx = ggml_init(params);
            // Tensor is 2D: [n_embd_s, mem_size]. Each cell = one row = ne[0] elements.
            int64_t cell_elements = s_l[il]->ne[0];
            size_t  cell_bytes    = s_l[il]->nb[1]; // row stride in bytes
...
bool llama_memory_recurrent::find_slot(const llama_ubatch & ubatch) {
    ...
    for (uint32_t s = 0; s < n_seqs; ++s) {
        ...
        if (!has_cell) {
            ...
            // Sequence already owns its cell — reuse it in place.
            // No checkpoint is created; recurrent state rollback for speculative
            // decoding is handled by seq_rm returning false when partial removal
            // would require rewinding tensor state.
        }
